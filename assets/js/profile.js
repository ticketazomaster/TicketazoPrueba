...
window.Profile = (() => {
  ...
  function _buildTicketUrl(ticket) {
    ...
    // Usamos "#code=" en lugar de "?code=" porque algunos servidores (Vercel, Serve) 
    // quitan los parámetros "?" al hacer redirecciones locales (.html a clean url)
    const code = ticket.qrCode || '';
    const isLocal = ['localhost', '127.0.0.1'].includes(window.location.hostname);
    const baseOrigin = isLocal ? 'http://192.168.0.151:8000' : window.location.origin;
    return `${baseOrigin}/verify.html#code=${encodeURIComponent(code)}`;
  }
  ...
  async function addTickets(eventId, qty, priceOverride, ticketTier, stripePaymentIntentId, seatLabels) {
    _hydrateState();
    ...
    if (_useDbTickets()) {
      const created = await DB.createTicketPurchase({
        ...
        seatLabels: Array.isArray(seatLabels) && seatLabels.length ? seatLabels : undefined,
      });
    ...
    for (let index = 0; index < qty; index += 1) {
      ...
      let tierLabel = ticketTier || 'General (GA)';
      if (seat) tierLabel = `${tierLabel} · ${seat}`;
      created.unshift({
        id: ticketId,
        eventId,
  ...
  function renderQRCode(ticketId) {
    ...
    if (typeof QRCode !== 'undefined') {
      ...
      // Poll database to auto-hide the QR code when it is consumed remotely
      window.__qrPolls = window.__qrPolls || {};
      if (window.__qrPolls[ticketId]) clearInterval(window.__qrPolls[ticketId]);

      const dbCode = ticket.qrCode;
      if (dbCode) {
        window.__qrPolls[ticketId] = setInterval(async () => {
          if (typeof DB === 'undefined' || !DB.client) return;
          const sb = DB.client();
          if (!sb) return;

          try {
            const { data } = await sb.from('boleto').select('usado, estado').eq('codigoqr', dbCode).maybeSingle();
            if (data && (data.usado === true || data.estado === 'usado')) {
              clearInterval(window.__qrPolls[ticketId]);
              wrap.innerHTML = `<div style="text-align:center;padding:10px;color:#ef4444;font-weight:bold;height:148px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(239,68,68,0.1);border-radius:12px;border:2px solid #ef4444"><span class="material-symbols-outlined" style="font-size:32px;margin-bottom:8px">block</span>YA UTILIZADO</div>`;

              // Opcional: Actualizar vista del ticket si fuese necesario
              const badge = document.querySelector(`#qr-${ticketId}`)?.parentElement?.querySelector('.ticket-status--access');
              if (badge) {
                badge.className = 'ticket-status ticket-status--access used';
                badge.textContent = 'Acceso Usado';
              }
            }
          } catch (e) { }
        }, 2500);
  ...
  function openTicket(ticketId) {
    ...
    // Renderiza la vista del ticket sin cambiar la URL a verify.html,
    // lo que rompía el router del SPA cuando corría en localhost.
